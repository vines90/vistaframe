...
const POINTS = [
  "Globally compliant systems with documented test reports.",
  "Intelligent fabrication workflow and repeatable quality gates.",
  "Certified assemblies for NFRC · CE · AS2047 · CSA · Energy Star · ISO pipelines.",
  "Engineering-grade documentation — shop drawings through delivery.",
];

export function ZeroError() {
  return (
    <section className="relative overflow-hidden bg-[var(--derchi-muted)] py-16 text-stone-200 lg:py-24">
      ...
      <div className="container relative mx-auto px-4 lg:px-8">
        <div className="grid gap-14 lg:grid-cols-[1fr_1.1fr] lg:gap-16 lg:items-center">
          <Reveal>
            <p className="derchi-eyebrow">Industry VistaFrame</p>
            <h2 className="derchi-title-rule mt-4 font-serif text-[clamp(1.875rem,4vw,2.75rem)] font-bold italic leading-snug tracking-tight text-white">
              <span className="not-italic text-[var(--accent-gold)]">Zero‑Error</span>
              <br />
              Intelligent manufacturing exceeds expectations.
            </h2>
            <Link
              href="/about"
            ...
            <ul className="space-y-6 border border-white/12 bg-black/35 p-6 backdrop-blur-sm lg:p-8">
              {POINTS.map((line) => (
                <li key={line} className="flex gap-4 border-b border-white/10 pb-5 last:border-0 last:pb-0">
                  ...
                  <span className="text-[15px] italic leading-relaxed text-stone-300">{line}</span>
                </li>
              ))}
            </ul>
          </Reveal>
